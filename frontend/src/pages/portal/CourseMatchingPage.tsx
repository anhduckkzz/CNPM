...
const CourseMatchingPage = () => {
  ...
  return (
    <div className="space-y-8">
          ...
          {data.history.map((course) => (
            <div key={course.id} className="flex flex-col rounded-[28px] border border-slate-100 p-5 shadow-soft">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <button className="rounded-2xl bg-primary px-4 py-3 font-semibold text-white shadow-soft" type="button">
                  {course.actionLabel}
                </button>
      ...
      <section className="rounded-[32px] bg-white p-8 shadow-soft">
        <div className="mx-auto max-w-3xl rounded-[32px] bg-slate-50 p-6 text-center">
          <p className="text-sm text-slate-500">Introduction to Programming · CO1002</p>
          <h3 className="mt-2 text-2xl font-semibold text-ink">Choose your preferred schedule</h3>
          <div className="mt-6 space-y-4 text-left">
            {data.modal.slots.map((slot) => (
              <div key={slot.id} className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
                  <div>
                    <p className="font-semibold text-ink">{slot.section}</p>
                    <p className="text-slate-500">Tutor: {slot.tutor}</p>
                  </div>
                ...
                <button
                  type="button"
                  className="mt-3 w-full rounded-2xl bg-primary px-4 py-2 font-semibold text-white"
                >
                  {slot.cta}
                </button>
              </div>
