...
### 1.1 Postgres configuration

The FastAPI app now reads portal state from Postgres instead of in-memory fixtures.

1. Provide a `DATABASE_URL` environment variable (e.g., `postgres://user:pass@host:5432/dbname`). Optional tuning knobs: `DB_POOL_MIN_SIZE`, `DB_POOL_MAX_SIZE`, `DB_SSLMODE`.
2. Seed/refresh the tables with the shipping fixtures:
   ...
   This script creates `portal_users` + `portal_bundles` tables and upserts the demo content. Re-run it whenever you need to sync fixtures into a clean database.

3. Start the API (`uvicorn app.main:app --reload`). The service will exit fast if `DATABASE_URL` is missing so misconfigurations surface early.

#### Starting Postgres on Render

1. In the Render dashboard choose **New → PostgreSQL** and pick the closest region/plan.
2. After the instance is provisioned, copy the `External Database URL`; Render already appends `?sslmode=require`, which works with psycopg.
3. Open your FastAPI service on Render and add an environment variable `DATABASE_URL` with that connection string (plus any optional pool vars).
4. Use Render’s “Shell” tab (or a one-off job) to seed data:
   ...
   Subsequent deploys can skip this step unless you drop tables.

5. Redeploy the backend to pick up the env var. The frontend (on Vercel) only needs the backend URL; no change required there.
